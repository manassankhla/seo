...
class ParserPool {
    ...
    init(size) {
        ...
        const target = size ?? Math.max(2, Math.min(8, cpuCount - 2));
        for (let i = 0; i < target; i++) {
            try {
                this.workers.push(this.spawnWorker(i));
            }
            catch (err) {
                logger.log('warn', 'main', `parser-pool: worker ${i} spawn failed: ${err instanceof Error ? err.message : String(err)}`);
            }
        }
        if (this.workers.length === 0) {
            logger.log('warn', 'main', 'parser-pool: no workers spawned — falling back to inline parseHtml.');
        }
        else {
            logger.log('info', 'main', `parser-pool: spawned ${this.workers.length} parser worker(s)`);
        }
    }
    ...
    spawnWorker(index) {
        ...
        w.on('exit', (code) => {
            ...
            if (code === 0)
                return;
            logger.log('warn', 'main', `parser-worker[${index}] exited (code=${code}) — respawning`);
            const respawn = this.tryRespawn(index);
            if (respawn)
                this.workers[index] = respawn;
        });
        return w;
    }
    tryRespawn(index) {
        ...
        try {
            ...
            logger.log('error', 'main', `parser-worker[${index}] respawn failed: ${err instanceof Error ? err.message : String(err)}`);
            return null;
        }
    }
    handleResponse(msg) {
