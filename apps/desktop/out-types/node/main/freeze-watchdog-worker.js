...
const mainStall = { active: false, startTs: 0, startOp: '' };
const readerStall = { active: false, startTs: 0, startOp: '' };
const rendererStall = { active: false, startTs: 0, startOp: '' };
...
function check() {
    ...
    if (mainHb > 0 && mainGap > STALL_THRESHOLD_MAIN_MS) {
        if (!mainStall.active) {
            ...
            appendLine(`${isoNow()} [STALL:MAIN start gap=${mainGap}ms] op="${mainOp || '<unknown>'}" ` +
        ...
        appendLine(`${isoNow()} [STALL:MAIN end after ${dur}ms] startOp="${mainStall.startOp}" endOp="${mainOp}"`);
    ...
    if (readerHb > 0 && readerGap > STALL_THRESHOLD_READER_MS) {
        if (!readerStall.active) {
            ...
            appendLine(`${isoNow()} [STALL:READER start gap=${readerGap}ms] op="${readerOp || '<unknown>'}"`);
        }
    }
    else if (readerStall.active) {
        const dur = now - readerStall.startTs;
        appendLine(`${isoNow()} [STALL:READER end after ${dur}ms] startOp="${readerStall.startOp}" endOp="${readerOp}"`);
    ...
    if (rendererStalled) {
        if (!rendererStall.active) {
            ...
            appendLine(`${isoNow()} [STALL:RENDERER start ${rendererReason}] last_main_op="${mainOp || '<unknown>'}" ` +
                `crawled=${counters.crawled} pending=${counters.pending}`);
        }
    }
    else if (rendererStall.active) {
        const dur = now - rendererStall.startTs;
        appendLine(`${isoNow()} [STALL:RENDERER end after ${dur}ms] last_lag=${rendererLag}ms`);
    ...
    if (!bootLogged) {
        bootLogged = true;
        appendLine(`${isoNow()} [BOOT] freeze-watchdog started ` +
    ...
    if (now - lastHeartbeatLogTs >= HEARTBEAT_LOG_INTERVAL_MS) {
        ...
        appendLine(`${isoNow()} [HEARTBEAT] main_op="${mainOp}" reader_op="${readerOp}" ` +
            `crawled=${counters.crawled} discovered=${counters.discovered} ` +
...
//# sourceMappingURL=freeze-watchdog-worker.js.map