...
class FreezeWatchdog {
    ...
    init(debugFilePath) {
        ...
        try {
            ...
            logger.log('warn', 'main', `freeze-watchdog: SharedArrayBuffer allocation failed (${err instanceof Error ? err.message : String(err)}) — diagnostics disabled.`);
            return;
        }
        try {
            ...
            logger.log('warn', 'main', `freeze-watchdog worker spawn failed: ${err instanceof Error ? err.message : String(err)} — diagnostics disabled.`);
            return;
        }
        this.state = state;
        ...
        this.worker.on('exit', (code) => {
            ...
            if (!this.terminated && code !== 0) {
                logger.log('warn', 'main', `freeze-watchdog worker exited unexpectedly (code=${code})`);
            }
        });
...
//# sourceMappingURL=freeze-watchdog.js.map