import { app, BrowserWindow, clipboard, ipcMain, dialog, Menu, Notification, shell, } from 'electron';
import { fileURLToPath } from 'node:url';
import { dirname, join } from 'node:path';
import { mkdirSync, readFileSync, writeFileSync, existsSync } from 'node:fs';
import { DEFAULT_CRAWL_CONFIG, IPC, } from '@freecrawl/shared-types';
import { Crawler, exportUrlsToCsv, exportUrlsToJson, exportUrlsToXml, exportSitemap, exportHtmlReport, compareCrawls, testUrlAgainstRobots, fetchSitemaps, validateSitemap, } from '@freecrawl/core';
 ...
 */
const shownDiagnosticDialogs = new Set();
/**
 ...
 */
function categorizeDiagnostic(msg) {
    ...
    if (/\bquery(A|AAAA|Soa|Srv|Mx|Txt|Ns|Cname|Any|Naptr|Ptr)\b/i.test(msg) && /ECONNREFUSED/.test(msg)) {
        return {
            ...
            detail: 'FreeCrawl already attempts to bypass broken system DNS automatically — if you see this dialog, even DNS-over-HTTPS over port 443 failed.\n\n' +
    ...
    if (/EDESTRUCTION/.test(msg)) {
        return {
            ...
            message: "Your system's DNS resolver crashed AND FreeCrawl's automatic DNS-over-HTTPS bypass also failed. This means the network stack is in a broken state — not just DNS.",
            detail: 'FreeCrawl normally recovers from a crashed Windows DNS Client by routing lookups through Cloudflare/Google over HTTPS:443. If you are seeing this dialog, that fallback also failed — usually because the operating-system network stack itself needs a reset.\n\n' +
    ...
    if (/UNABLE_TO_GET_ISSUER_CERT_LOCALLY|SELF_SIGNED_CERT_IN_CHAIN|UNABLE_TO_VERIFY_LEAF_SIGNATURE|DEPTH_ZERO_SELF_SIGNED_CERT/.test(msg)) {
        return {
            ...
            detail: 'Common culprits: Kaspersky, ESET, Bitdefender, Zscaler, BlueCoat, Fortigate.\n\n' +
                'Try one of these:\n' +
...
function diagnosticDialogPrefKey(key) {
    return `skipDiag:${key}`;
}
function isDontShowAgain(key) {
    loadPrefs();
 ...
 */
function showDiagnosticDialog(diag) {
...
function maybeShowDiagnosticDialog(msg) {
    const diag = categorizeDiagnostic(msg);
    if (!diag)
        return;
    if (shownDiagnosticDialogs.has(diag.key))
        return;
    if (isDontShowAgain(diag.key))
        return;
 ...
 */
async function parseHtmlViaPool(html, pageUrl, opts) {
    ...
    try {
        ...
        logger.log('warn', 'main', `parser-pool dispatch failed (${err instanceof Error ? err.message : String(err)}) — using inline parseHtml as fallback.`);
        return inlineParseHtml(html, pageUrl, opts);
    }
}
/**
 ...
 */
async function writeFetchedUrlViaPool(payload) {
    if (!dbWriterPool.isReady()) {
        return getDb().writeFetchedUrl(payload);
    }
    try {
        return await dbWriterPool.call('writeFetchedUrl', [payload]);
    }
    catch (err) {
        logger.log('warn', 'main', `db-writer dispatch failed (${err instanceof Error ? err.message : String(err)}) — falling back to main-thread writeFetchedUrl.`);
        return getDb().writeFetchedUrl(payload);
    }
}
function getDb() {
    if (!db) {
        ...
        try {
            ...
            if (raw)
                lastCrawlConfig = JSON.parse(raw);
        }
        catch {
        ...
        try {
            ...
            logger.log('warn', 'main', `db-reader pool init failed: ${err instanceof Error ? err.message : String(err)} — falling back to main-thread queries.`);
        }
        try {
            dbWriterPool.init(defaultPath, freezeWatchdog.sharedBuffer);
        }
        catch (err) {
            logger.log('warn', 'main', `db-writer pool init failed: ${err instanceof Error ? err.message : String(err)} — writes fall back to main thread.`);
        }
    }
    return db;
}
 ...
 */
function openProjectAtPath(filePath) {
    if (activeCrawler) {
        activeCrawler.stop();
        activeCrawler = null;
    }
    ...
    try {
        ...
        logger.log('warn', 'main', `db-reader pool swap failed: ${err instanceof Error ? err.message : String(err)}`);
    }
    try {
        dbWriterPool.swap(filePath, freezeWatchdog.sharedBuffer);
    }
    catch (err) {
        logger.log('warn', 'main', `db-writer pool swap failed: ${err instanceof Error ? err.message : String(err)}`);
    }
...
function getRecentProjects() {
    ...
    if (!Array.isArray(raw))
        return [];
    return raw.filter((p) => typeof p === 'string' && p.length > 0).slice(0, 10);
}
    ...
    Menu.setApplicationMenu(buildAppMenu({
        ...
        onOpenRecent: (path) => {
            try {
                ...
                dialog.showErrorBox('Open Project Failed', `Could not open ${path}.\n\n${err.message}`);
 ...
 */
function resetDiagnosticDialogs() {
    loadPrefs();
    let removed = 0;
    ...
    if (removed > 0)
        schedulePrefsWrite();
    logger.log('info', 'main', `Diagnostic warnings reset (${removed} suppressed dialog${removed === 1 ? '' : 's'} re-enabled).`);
    if (mainWindow) {
        void dialog.showMessageBox(mainWindow, {
            type: 'info',
...
async function promptOpenProject() {
    ...
    try {
        ...
        dialog.showErrorBox('Open Project Failed', `Could not open the selected file.\n\n${err.message}`);
    }
}
...
function openLogsWindow() {
    ...
    const setBusy = (busy) => {
        ...
        isBusy = busy;
        if (!win.isDestroyed())
            win.webContents.send(IPC.logsBusy, busy);
    };
    const markBusy = () => {
        const now = Date.now();
        if (isBusy && now - lastBusyTickMs < 150)
            return; // already busy + recent tick
        lastBusyTickMs = now;
        setBusy(true);
        if (busyTimer)
...
function registerIpc() {
    ...
    ipcMain.on(IPC.rendererLagReport, (_e, lagMs) => {
    ...
    ipcMain.handle(IPC.robotsTest, (_e, input) => testUrlAgainstRobots(input.url, input.userAgent, input.customRobots));
    ipcMain.handle(IPC.sitemapValidate, async (_e, input) => {
    ...
    ipcMain.handle(IPC.reportsPagesPerDirectory, (_e, input) => callReaderOrFallback('getPagesPerDirectory', [{ depth: input.depth, limit: input.limit }], () => getDb().getPagesPerDirectory({ depth: input.depth, limit: input.limit })));
    ipcMain.handle(IPC.reportsStatusCodeHistogram, () => callReaderOrFallback('getStatusCodeHistogram', [], () => getDb().getStatusCodeHistogram()));
    ipcMain.handle(IPC.reportsDepthHistogram, () => callReaderOrFallback('getDepthHistogram', [], () => getDb().getDepthHistogram()));
    ipcMain.handle(IPC.reportsResponseTimeHistogram, () => callReaderOrFallback('getResponseTimeHistogram', [], () => getDb().getResponseTimeHistogram()));
    ipcMain.handle(IPC.reportsTopUrls, (_e, input) => {
        const limit = Math.min(500, Math.max(1, input.limit ?? 25));
        const column = input.metric === 'response-time'
            ? 'response_time_ms'
            : input.metric === 'inlinks'
                ...
                : input.metric === 'outlinks'
                    ...
                    : input.metric === 'depth'
                        ? 'depth'
                        : 'content_length';
        return callReaderOrFallback('topUrlsBy', [column, limit], () => getDb().topUrlsBy(column, limit));
    });
    ipcMain.handle(IPC.reportsExternalDomainHealth, (_e, limit) => callReaderOrFallback('externalDomainHealth', [limit ?? 100], () => getDb().externalDomainHealth(limit ?? 100)));
    ipcMain.handle(IPC.reportsAnalyticsCoverage, () => callReaderOrFallback('analyticsCoverage', [], () => getDb().analyticsCoverage()));
    ipcMain.handle(IPC.reportsLinkPositions, () => callReaderOrFallback('linkPositionBreakdown', [], () => getDb().linkPositionBreakdown()));
    ipcMain.handle(IPC.reportsImageWeightPerPage, (_e, limit) => callReaderOrFallback('imageWeightPerPage', [limit ?? 25], () => getDb().imageWeightPerPage(limit ?? 25)));
    ipcMain.handle(IPC.reportsInlinksHistogram, () => callReaderOrFallback('inlinksHistogram', [], () => getDb().inlinksHistogram()));
    ipcMain.handle(IPC.reportsWordCountHistogram, () => callReaderOrFallback('wordCountHistogram', [], () => getDb().wordCountHistogram()));
    ipcMain.handle(IPC.reportsUrlLengthHistogram, () => callReaderOrFallback('urlLengthHistogram', [], () => getDb().urlLengthHistogram()));
    ipcMain.handle(IPC.reportsWordCountPerDirectory, (_e, input) => callReaderOrFallback('wordCountPerDirectory', [{ depth: input.depth, limit: input.limit }], () => getDb().wordCountPerDirectory({ depth: input.depth, limit: input.limit })));
    ipcMain.handle(IPC.reportsSitemapOrphans, (_e, limit) => callReaderOrFallback('sitemapOrphans', [limit ?? 1000], () => getDb().sitemapOrphans(limit ?? 1000)));
    ipcMain.handle(IPC.reportsServerHeaders, () => callReaderOrFallback('serverHeaderBreakdown', [], () => getDb().serverHeaderBreakdown()));
    ipcMain.handle(IPC.prefsExportSettings, async (_e, input) => {
    ...
    ipcMain.handle(IPC.prefsImportSettings, async () => {
        if (!mainWindow)
        ...
        try {
            ...
            dialog.showErrorBox('Import Failed', `Cannot parse JSON: ${err.message}`);
            return { filePath: '', config: null, unknownFields: [] };
        }
        // Accept both the wrapped envelope and a bare CrawlConfig object —
        // bare objects are useful for hand-edited setting fragments.
        const config = raw && typeof raw === 'object' && 'config' in raw
            ? raw.config
            : raw;
        if (!config || typeof config !== 'object') {
            dialog.showErrorBox('Import Failed', 'Imported file does not contain a settings object.');
            return { filePath: '', config: null, unknownFields: [] };
        }
    ...
    ipcMain.handle(IPC.prefsSet, (_e, key, value) => {
        loadPrefs();
        prefsCache[key] = value;
        schedulePrefsWrite();
    });
    ipcMain.handle(IPC.prefsDelete, (_e, key) => {
        loadPrefs();
    ...
    function attachCrawlerListeners(crawler) {
        crawler.on('progress', (p) => {
            if (activeCrawler !== crawler)
                return;
        ...
        crawler.on('done', (summary) => {
            if (activeCrawler !== crawler)
                return;
            logger.log('info', 'crawler', `Crawl done: total=${summary.total} avgResp=${summary.avgResponseTimeMs}ms totalBytes=${summary.totalBytes}`);
        ...
        crawler.on('debug', (msg) => {
            if (activeCrawler !== crawler)
                return;
            logger.log('debug', 'crawler', msg);
        });
    }
    ipcMain.handle(IPC.crawlStart, (_e, config) => {
        if (activeCrawler) {
            activeCrawler.stop();
        ...
        shownDiagnosticDialogs.clear();
        logger.log('info', 'crawler', `Crawl starting: ${config.startUrl} (scope=${config.scope}, maxDepth=${config.maxDepth}, maxUrls=${config.maxUrls}, concurrency=${config.maxConcurrency}, rps=${config.maxRps})`);
        ...
        const crawler = new Crawler(config, database, {
            setOp: (op) => freezeWatchdog.setMainOp(op),
    ...
    ipcMain.handle(IPC.crawlAddUrl, (_e, url) => {
        if (!activeCrawler)
    ...
    ipcMain.handle(IPC.projectSaveAs, async () => {
        const win = mainWindow;
        if (!win)
            return null;
        ...
        try {
            ...
            logger.log('warn', 'main', `Save Project As: reopen failed: ${err.message}`);
        }
        return { filePath: target, bytesWritten: bytes };
    });
    ipcMain.handle(IPC.projectOpen, async (_e, filePath) => {
        let target = filePath;
        if (!target) {
            if (!mainWindow)
                return null;
        ...
        try {
            ...
            if (mainWindow) {
                dialog.showErrorBox('Open Project Failed', `Could not open ${target}.\n\n${err.message}`);
            }
            return null;
        }
    });
    ipcMain.handle(IPC.projectCurrentPath, () => {
        return currentProjectPath || null;
    });
    ipcMain.handle(IPC.confirmClear, async () => {
        const win = mainWindow;
        if (!win)
            return { confirmed: false, skipNext: false };
        const res = await dialog.showMessageBox(win, {
            ...
            detail: 'This will remove all discovered URLs, links, and crawl metadata for this project. This action cannot be undone.',
    ...
    const yieldThenRun = (fn) => new Promise((resolve, reject) => {
        setImmediate(() => {
            try {
                resolve(fn());
            }
    ...
    ipcMain.handle(IPC.urlsQuery, async (_e, input) => {
        const args = [
            {
                limit: input.limit,
                offset: input.offset,
        ...
        return callReaderOrFallback('queryUrls', args, () => getDb().queryUrls(args[0]));
    });
    ipcMain.handle(IPC.overviewGet, async () => callReaderOrFallback('getOverviewCounts', [], () => getDb().getOverviewCountsAsync()));
    ipcMain.handle(IPC.imagesQuery, async (_e, input) => {
        const args = [
            {
                limit: input.limit,
                offset: input.offset,
        ...
        return callReaderOrFallback('queryImages', args, () => getDb().queryImages(args[0]));
    });
    ipcMain.handle(IPC.brokenLinksQuery, async (_e, input) => {
        const args = [
            {
                limit: input.limit,
                offset: input.offset,
        ...
        return callReaderOrFallback('queryBrokenLinks', args, () => getDb().queryBrokenLinks(args[0]));
    });
    ipcMain.handle(IPC.urlContextMenu, (e, input) => {
    ...
    ipcMain.handle(IPC.urlBulkContextMenu, async (e, input) => {
        const win = BrowserWindow.fromWebContents(e.sender);
        const db = getDb();
        ...
        const template = [
            ...
            },
            {
                ...
                toolTip: urls.length > 20 ? 'Limited to 20 URLs to avoid spawning too many tabs' : undefined,
                click: () => {
                    for (const u of urls)
    ...
    ipcMain.handle(IPC.urlDetailGet, async (_e, input) => callReaderOrFallback('getUrlDetail', [input.id, input.linkLimit ?? 500], () => getDb().getUrlDetail(input.id, input.linkLimit ?? 500)));
    ipcMain.handle(IPC.urlSourceGet, (_e, input) => {
    ...
    ipcMain.handle(IPC.urlPageImages, (_e, input) => {
        const rows = getDb().pageImagesDetailed(input.id, input.limit ?? 5000);
        return { rows };
    });
    ipcMain.handle(IPC.urlCertInfo, (_e, input) => {
    ...
    ipcMain.handle(IPC.summaryGet, () => callReaderOrFallback('getSummary', [], () => getDb().getSummary()));
    ipcMain.handle(IPC.exportCsv, async (_e, input) => {
        let filePath = input.filePath;
    ...
    ipcMain.handle(IPC.exportJson, async (_e, input) => {
        let filePath = input.filePath;
        const isSelection = (input.selectedIds?.length ?? 0) > 0;
        if (!filePath) {
            const res = await dialog.showSaveDialog(mainWindow, {
    ...
    ipcMain.handle(IPC.exportXml, async (_e, input) => {
        let filePath = input.filePath;
        const isSelection = (input.selectedIds?.length ?? 0) > 0;
        if (!filePath) {
            const res = await dialog.showSaveDialog(mainWindow, {
    ...
    ipcMain.handle(IPC.dataDeleteByDomain, async (_e, input) => {
    ...
    ipcMain.handle(IPC.crashRecoveryStatus, async () => {
    ...
    ipcMain.handle(IPC.crashRecoveryResume, async () => {
        if (activeCrawler)
            return { accepted: false };
        if (pendingRecoveryCheckpoint.length === 0)
            return { accepted: false };
    ...
    ipcMain.handle(IPC.crashRecoveryDiscard, async () => {
        pendingRecoveryCheckpoint = [];
        ...
        try {
            ...
            logger.log('warn', 'main', `clearQueueCheckpoint failed on discard: ${err instanceof Error ? err.message : String(err)}`);
        }
    });
    ipcMain.handle(IPC.exportBulk, async () => {
        if (!mainWindow) {
    ...
    ipcMain.handle(IPC.exportHtmlReport, async (_e, input) => {
        let filePath = input.filePath;
        if (!filePath) {
            const res = await dialog.showSaveDialog(mainWindow, {
    ...
    ipcMain.handle(IPC.sitemapGenerate, async (_e, input) => {
        let filePath = input.filePath;
        const variant = input.variant ?? 'standard';
        const gzip = input.gzip ?? false;
        if (!filePath) {
            const baseName = variant === 'image'
                ? 'sitemap-images.xml'
                : variant === 'hreflang'
        ...
        if (mainWindow) {
            ...
            await dialog.showMessageBox(mainWindow, {
                ...
                message: result.sharded
                    ? `Sharded sitemap written: ${result.urlsWritten.toLocaleString()} URLs across ${result.files.length - 1} parts + index.`
                    : `Sitemap written with ${result.urlsWritten.toLocaleString()} URLs${result.truncated ? ' (truncated at the 50,000 limit).' : '.'}`,
                detail,
                buttons: ['OK'],
                noLink: true,
            });
    ...
    ipcMain.handle(IPC.compareLoad, async (_e, input) => {
        let filePath = input.filePath;
        if (!filePath) {
            const res = await dialog.showOpenDialog(mainWindow, {
    ...
    ipcMain.handle(IPC.graphSnapshot, (_e, input) => {
        return getDb().graphSnapshot(input.nodeLimit ?? 1000);
    });
    ipcMain.handle(IPC.topAnchorTexts, (_e, limit) => {
...
void app.whenReady().then(() => {
    ...
    try {
        ...
        if (logFile)
            logger.log('info', 'main', `Disk log file: ${logFile}`);
    }
    catch (err) {
        logger.log('warn', 'main', `Disk logging unavailable: ${err.message}`);
    }
    ...
    try {
        ...
        logger.log('warn', 'main', `freeze-watchdog init failed: ${err.message}`);
    }
    ...
    try {
        ...
        logger.log('warn', 'main', `parser-pool init failed: ${err.message} — falling back to inline parseHtml.`);
    }
    loadPrefs();
    rebuildMenu();
    registerIpc();
