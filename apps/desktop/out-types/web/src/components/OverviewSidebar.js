...
export function OverviewSidebar() {
    ...
    useEffect(() => {
        ...
        if (bucket === lastRefetchAtRef.current)
            return;
    ...
    return (_jsxs("div", { className: "flex h-full flex-col bg-surface-900", children: [_jsxs("div", { className: "flex items-center border-b border-surface-800 bg-surface-850 px-2 py-1.5", children: [_jsx("div", { className: "text-[10px] font-semibold uppercase tracking-wide text-surface-400", children: "Overview" }), _jsxs("div", { className: "ml-auto flex items-center gap-3 text-[10px] text-surface-500", children: [_jsx("span", { children: "URLs" }), _jsx("span", { children: "% of Total" })] })] }), _jsx("div", { className: "flex-1 overflow-auto", children: _jsx("div", { className: "py-1 text-[11px]", children: tree.map((node) => (_jsx(TreeNode, { node: node, depth: 0, expanded: expanded, toggle: toggle, activeCategory: activeCategory, onClick: navigateToCategory, total: totalForPercent }, node.key))) }) })] }));
}
const TreeNode = memo(function TreeNode({ node, depth, expanded, toggle, activeCategory, onClick, total, }) {
    const isExpanded = expanded.has(node.key);
    const isActive = node.category && node.category === activeCategory;
    const hasChildren = (node.children?.length ?? 0) > 0;
    return (_jsxs(_Fragment, { children: [_jsxs("div", { className: clsx('group flex cursor-pointer items-center gap-1 px-2 py-1 hover:bg-surface-800', isActive && 'bg-accent-500/20 text-surface-50'), style: { paddingLeft: 8 + depth * 12 }, onClick: () => {
                    if (hasChildren)
                        toggle(node.key);
                    if (node.category)
                        onClick(node.category);
                }, children: [hasChildren ? (isExpanded ? (_jsx(ChevronDown, { className: "h-3 w-3 shrink-0 text-surface-500" })) : (_jsx(ChevronRight, { className: "h-3 w-3 shrink-0 text-surface-500" }))) : (_jsx("span", { className: "w-3 shrink-0" })), _jsx("span", { className: "min-w-0 flex-1 truncate text-surface-200", children: node.label }), node.count !== undefined && (_jsxs(_Fragment, { children: [_jsx("span", { className: "font-mono tabular-nums text-surface-300", children: node.count.toLocaleString() }), _jsx("span", { className: "w-14 text-right font-mono tabular-nums text-surface-500", children: total > 0 ? ((node.count / total) * 100).toFixed(2) + '%' : '—' })] }))] }), isExpanded &&
                node.children?.map((child) => (_jsx(TreeNode, { node: child, depth: depth + 1, expanded: expanded, toggle: toggle, activeCategory: activeCategory, onClick: onClick, total: total }, child.key)))] }));
});
...
//# sourceMappingURL=OverviewSidebar.js.map