 ...
 */
export function useLazyUrlRows(opts) {
    const [total, setTotal] = useState(0);
    const [version, setVersion] = useState(0);
    const chunks = useRef(new Map());
    const chunkOrder = useRef([]);
    const fetching = useRef(new Set());
    const activeRange = useRef({ first: 0, last: 0 });
    ...
    const queryChunk = useCallback((chunkIdx) => window.freecrawl.urlsQuery({
        limit: CHUNK_SIZE,
    ...
    const queryMeta = useCallback(() => window.freecrawl.urlsQuery({
        limit: 0,
        offset: 0,
        category: opts.category,
        search: opts.search || undefined,
    ...
    const fetchChunk = useCallback(async (chunkIdx) => {
        if (chunks.current.has(chunkIdx))
            return;
        if (fetching.current.has(chunkIdx))
            return;
        fetching.current.add(chunkIdx);
        const token = resetToken.current;
        try {
            const { rows, total: t } = await queryChunk(chunkIdx);
            if (token !== resetToken.current)
                return;
    ...
    const ensureRange = useCallback((start, end) => {
        ...
        for (let i = first; i <= last; i++) {
            if (!chunks.current.has(i))
                void fetchChunk(i);
        }
    }, [fetchChunk]);
    const rowAt = useCallback((index) => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [version]);
    const loadedRows = useMemo(() => [...chunks.current.values()].reduce((n, c) => n + c.length, 0), 
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [version]);
    return { total, loadedRows, rowAt, ensureRange };
}
//# sourceMappingURL=useLazyUrlRows.js.map