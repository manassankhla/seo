...
import { COLUMN_SPECS, columnId } from './columns.js';
...
export function UrlsTab() {
    ...
    const [columnWidths, setColumnWidths] = useState(() => loadStoredWidths(activeTab));
    ...
    const getWidth = useCallback((c) => columnWidths[columnId(c)] ?? c.size, [columnWidths]);
    const rowNumWidth = columnWidths[ROW_NUM_KEY] ?? ROW_NUM_DEFAULT_WIDTH;
    const totalWidth = useMemo(() => rowNumWidth + columns.reduce((n, c) => n + getWidth(c), 0), [columns, getWidth, rowNumWidth]);
    ...
    const handleRowClick = (rowId, rowIndex, e) => {
    ...
    const handleCellClick = (rowId, colIdx, rowIndex, e) => {
        ...
        if (e.shiftKey && cellAnchor.current && cellAnchor.current.col === colIdx) {
            ...
            const [lo, hi] = anchor.index < rowIndex ? [anchor.index, rowIndex] : [rowIndex, anchor.index];
    ...
    const handleHeaderClick = (colIdx, e) => {
    ...
    const applyCellDrag = useCallback((toIdx, toCol) => {
        ...
        if (endRow)
            setSelectedUrlId(endRow.id);
    }, [lazy, setSelectedUrlId]);
    const applyRowDrag = useCallback((toIdx) => {
        const d = dragRef.current;
        if (!d || d.kind !== 'row')
            return;
        d.idx = toIdx;
    ...
    const applyColumnDrag = useCallback((toCol) => {
        const d = dragRef.current;
        if (!d || d.kind !== 'column')
            return;
        d.col = toCol;
    ...
    const beginCellDrag = (rowId, idx, colIdx, e) => {
        if (e.button !== 0)
            return;
    ...
    const beginRowDrag = (rowId, idx, e) => {
        if (e.button !== 0)
            return;
        if (e.shiftKey) {
    ...
    const beginColumnDrag = (colIdx, e) => {
        if (e.button !== 0)
            return;
        if (e.shiftKey) {
    ...
    const startResize = (colKey, startWidth, clientX) => {
    ...
    return (_jsxs("div", { className: "flex h-full flex-col", children: [_jsxs("div", { className: "flex shrink-0 items-center gap-2 border-b border-surface-800 bg-surface-900/30 px-3 py-1.5", children: [_jsx("input", { className: "input w-96", placeholder: "Search URLs / titles\u2026", value: search, onChange: (e) => setSearch(e.target.value), spellCheck: false }), _jsxs("button", { type: "button", onClick: () => setFilterDialogOpen(true), className: clsx('inline-flex items-center gap-1.5 rounded border px-2 py-1 text-[11px] transition', activeClauseCount > 0
                            ? 'border-accent-500/60 bg-accent-500/15 text-accent-300 hover:bg-accent-500/25'
                            : 'border-surface-700 text-surface-300 hover:bg-surface-800'), title: "Advanced filter", children: [_jsx(Filter, { className: "h-3.5 w-3.5" }), _jsx("span", { children: "Advanced" }), activeClauseCount > 0 && (_jsx("span", { className: "rounded bg-accent-500/20 px-1 font-mono text-[10px]", children: activeClauseCount }))] }), activeClauseCount > 0 && (_jsx("button", { type: "button", onClick: () => setFilter(null), className: "inline-flex items-center gap-1 rounded border border-surface-700 px-1.5 py-1 text-[11px] text-surface-400 hover:bg-surface-800 hover:text-surface-200", title: "Clear advanced filter", children: _jsx(X, { className: "h-3 w-3" }) })), _jsxs("div", { className: "ml-auto text-[11px] text-surface-500", children: [_jsx("span", { className: "font-mono text-surface-200", children: lazy.total.toLocaleString() }), " URLs", _jsxs("span", { className: "ml-2 text-surface-600", children: ["(", lazy.loadedRows.toLocaleString(), " loaded)"] })] })] }), _jsxs("div", { ref: scrollRef, className: "relative flex-1 select-none overflow-auto", children: [_jsxs("div", { style: { minWidth: totalWidth, width: '100%' }, children: [_jsxs("div", { className: "sticky top-0 z-10 flex bg-surface-900 text-[11px]", style: { minWidth: totalWidth, width: '100%', height: HEADER_HEIGHT }, children: [_jsxs("div", { className: "relative flex select-none items-center justify-end border-b border-r border-surface-800 px-2 font-medium text-surface-400", style: {
                                            width: rowNumWidth,
                                            minWidth: rowNumWidth,
                                            flex: `0 0 ${rowNumWidth}px`,
                                        }, title: "Row number", children: ["Row", _jsx("div", { className: "absolute -right-1 top-0 bottom-0 z-20 w-2 cursor-col-resize group hover:bg-accent-500/40", onMouseDown: (e) => {
                                                    e.preventDefault();
                                                    e.stopPropagation();
                                                ...
                                                }, onClick: (e) => e.stopPropagation(), title: "Drag to resize \u00B7 double-click to reset" })] }), columns.map((c, colIdx) => {
                                        const w = getWidth(c);
                                        const id = columnId(c);
                                        const isColSelected = selectedColumns.has(colIdx);
                                        const isSortCol = sortBy === c.key;
                                        return (_jsxs("div", { className: clsx('group/header relative flex select-none items-center gap-1 border-b border-r border-surface-800 pl-2 pr-1 font-medium', isColSelected
                                                ? 'bg-accent-500/25 text-surface-50'
                                                : 'text-surface-300 hover:text-surface-100'), style: { width: w, minWidth: w, flex: `0 0 ${w}px` }, onMouseDown: (e) => beginColumnDrag(colIdx, e), onMouseEnter: () => {
                                                if (dragRef.current?.kind === 'column')
                                                    applyColumnDrag(colIdx);
                                            }, title: c.header + ' (click to select column · drag to select multiple · click arrow to sort)', children: [_jsx("span", { className: "cursor-pointer truncate", children: c.header }), (c.info || c.example) && (_jsx("span", { className: "shrink-0", onMouseDown: (e) => e.stopPropagation(), onClick: (e) => e.stopPropagation(), children: _jsx(InfoTip, { info: c.info, example: c.example }) })), _jsx("button", { type: "button", className: clsx('ml-auto flex shrink-0 items-center rounded px-0.5 hover:bg-surface-800', isSortCol ? 'text-accent-300' : 'text-surface-600 hover:text-surface-300'), onClick: (e) => {
                                                        e.stopPropagation();
                                                        handleSort(c.key);
                                                    }, title: isSortCol
                                                        ? `Sorted ${sortDir === 'asc' ? 'ascending' : 'descending'} — click to flip`
                                                        : 'Sort by this column', children: isSortCol ? (_jsx("span", { className: "text-[10px]", children: sortDir === 'asc' ? '▲' : '▼' })) : (_jsx(ChevronsUpDown, { className: "h-3 w-3" })) }), _jsx("div", { className: "absolute -right-1 top-0 bottom-0 z-20 w-2 cursor-col-resize hover:bg-accent-500/40", onMouseDown: (e) => {
                                                        e.preventDefault();
                                                        e.stopPropagation();
                                                        ...
                                                        e.stopPropagation();
                                                        resetColumn(id);
                                                    }, onClick: (e) => e.stopPropagation(), title: "Drag to resize \u00B7 double-click to reset" })] }, id));
                                    }), _jsx("div", { className: "flex-1 border-b border-surface-800" })] }), _jsx("div", { className: "relative", style: { height: virtualizer.getTotalSize(), minWidth: totalWidth, width: '100%' }, children: virtualRows.map((vi) => {
                                    const row = lazy.rowAt(vi.index);
                                    const rowSelected = row !== null && selectedIds.has(row.id);
                                    return (_jsxs("div", { "data-index": vi.index, ref: virtualizer.measureElement, onContextMenu: (e) => {
                                            e.preventDefault();
                                            if (!row)
                                                return;
                                        ...
                                        }, className: clsx('absolute left-0 top-0 flex items-center border-b border-surface-900 text-[11px]', rowSelected ? 'bg-accent-500/15' : 'hover:bg-surface-900/30'), style: {
                                            transform: `translateY(${vi.start}px)`,
                                            height: ROW_HEIGHT,
                                            minWidth: totalWidth,
                                            width: '100%',
                                        }, children: [_jsx("div", { className: clsx('flex cursor-pointer items-center justify-end overflow-hidden border-r border-surface-900 px-2 font-mono tabular-nums', rowSelected
                                                    ? 'bg-accent-500/30 text-surface-50'
                                                    : 'text-surface-500 hover:bg-surface-800/60'), style: {
                                                    width: rowNumWidth,
                                                    minWidth: rowNumWidth,
                                                    flex: `0 0 ${rowNumWidth}px`,
                                                    height: '100%',
                                                    ...
                                                    if (row)
                                                        beginRowDrag(row.id, vi.index, e);
                                                }, onMouseEnter: () => {
                                                    if (dragRef.current?.kind === 'row')
                                                        applyRowDrag(vi.index);
                                                }, title: "Click to select row \u00B7 drag to select multiple rows", children: vi.index + 1 }), columns.map((c, colIdx) => {
                                                const w = getWidth(c);
                                                const cellSel = row !== null &&
                                                    (selectedCells.has(`${row.id}:${colIdx}`) ||
                                                        selectedColumns.has(colIdx));
                                                return (_jsx("div", { className: clsx('flex cursor-cell items-center overflow-hidden border-r border-surface-900 px-2', cellSel
                                                        ? 'bg-accent-500/30 text-surface-50'
                                                        : 'hover:bg-surface-800/40'), style: {
                                                        width: w,
                                                        minWidth: w,
                                                        flex: `0 0 ${w}px`,
                                                        height: '100%',
                                                        ...
                                                        if (row)
                                                            beginCellDrag(row.id, vi.index, colIdx, e);
                                                    }, onMouseEnter: () => {
                                                        if (dragRef.current?.kind === 'cell') {
                                                            applyCellDrag(vi.index, colIdx);
                                                        }
                                                    }, children: _jsx(Cell, { row: row, spec: c }) }, columnId(c)));
                                            }), _jsx("div", { className: "flex-1" })] }, vi.key));
                                }) })] }), lazy.total === 0 && (_jsx("div", { className: "pointer-events-none absolute inset-0 flex items-center justify-center", style: { top: HEADER_HEIGHT }, children: _jsxs("div", { className: "max-w-md text-center", children: [_jsx("div", { className: "mb-1 text-sm font-semibold text-surface-300", children: "No URLs to show" }), _jsx("div", { className: "text-xs text-surface-500", children: "Start a crawl or choose a different category." })] }) }))] }), _jsxs("div", { className: "flex shrink-0 items-center justify-end gap-4 border-t border-surface-800 bg-surface-900/60 px-3 text-[11px] text-surface-400", style: { height: STATUS_BAR_HEIGHT }, children: [_jsxs("span", { children: ["Selected Rows:", ' ', _jsx("span", { className: "font-mono tabular-nums text-surface-200", children: selectedIds.size.toLocaleString() })] }), _jsxs("span", { children: ["Selected Cells:", ' ', _jsx("span", { className: "font-mono tabular-nums text-surface-200", children: selectedCells.size.toLocaleString() })] }), _jsxs("span", { children: ["Selected Columns:", ' ', _jsx("span", { className: "font-mono tabular-nums text-surface-200", children: selectedColumns.size.toLocaleString() })] }), _jsxs("span", { children: ["Filter Total:", ' ', _jsx("span", { className: "font-mono tabular-nums text-surface-200", children: lazy.total.toLocaleString() })] })] }), _jsx(AdvancedFilterDialog, { open: filterDialogOpen, initial: filter, onClose: () => setFilterDialogOpen(false), onApply: (f) => setFilter(f) })] }));
}
function Cell({ row, spec }) {
    ...
    if (spec.kind === 'status') {
        ...
        return (_jsx("span", { className: clsx('inline-block rounded px-1.5 font-mono text-[10px]', statusClasses(code)), children: code ?? '—' }));
    }
    if (spec.kind === 'indexability') {
        const v = row.indexability;
        return (_jsx("span", { className: clsx('truncate', v === 'indexable' ? 'text-emerald-400' : 'text-amber-400'), title: v, children: v === 'indexable' ? 'Indexable' : 'Non-Indexable' }));
    }
    if (spec.kind === 'indexability-status') {
        ...
        return (_jsx("span", { className: "block truncate text-surface-200", title: label, children: label }));
    }
    if (spec.kind === 'number') {
        return (_jsx("span", { className: "block truncate font-mono tabular-nums text-surface-200", children: raw === null || raw === undefined ? '—' : Number(raw).toLocaleString() }));
    }
    if (spec.kind === 'mono') {
        return (_jsx("span", { className: "block truncate font-mono text-surface-100", title: value, children: value || _jsx("span", { className: "text-surface-700", children: "\u2014" }) }));
    }
    return (_jsx("span", { className: "block truncate text-surface-200", title: value, children: value || _jsx("span", { className: "text-surface-700", children: "\u2014" }) }));
}
function indexabilityStatusLabel(v) {
    switch (v) {
        case 'indexable':
            return '';
...
//# sourceMappingURL=UrlsTab.js.map